...
const ArticlesService = {
  getAllArticles(db) {
    return db
      ...
      .select(
        ...
        'art.content',
        db.raw(
          `count(DISTINCT comm) AS number_of_updates`
        ),
        db.raw(
          `json_strip_nulls(
            json_build_object(
  ...
  getUpdatesForArticle(db, article_id) {
    return db
      .from('blogful_updates AS comm')
      .select(
        'comm.id',
        'comm.text',
        'comm.date_created',
  ...
  serializeArticle(article) {
    ...
    return {
      ...
      number_of_updates: Number(article.number_of_updates) || 0,
      author: {
        id: author.id,
